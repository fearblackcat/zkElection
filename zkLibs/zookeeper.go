...
func New(endpoints []string, options *Config) (Store, error) {
	s := &Zookeeper{}
...
func (s *Zookeeper) Get(key string, opts *ReadOptions) (pair *KVPair, err error) {
...
func (s *Zookeeper) CreateSequenceEphemeralPath(path string, data []byte) (string, error) {
	newpath := "/" + path

	seqPath, err := s.client.CreateProtectedEphemeralSequential(newpath, data, zk.WorldACL(zk.PermAll))
	if err != nil {
		// Skip if node already exists
		if err != zk.ErrNodeExists {
			return seqPath, err
...
func (s *Zookeeper) Put(key string, value []byte, opts *WriteOptions) error {
	...
	if !exists {
		if opts != nil && opts.TTL > 0 {
			s.createFullPath(SplitKey(strings.TrimSuffix(key, "/")), value, true)
		} else {
			s.createFullPath(SplitKey(strings.TrimSuffix(key, "/")), value, false)
		}
	} else {
...
func (s *Zookeeper) Exists(key string, opts *ReadOptions) (bool, error) {
...
func (s *Zookeeper) Watch(key string, stopCh <-chan struct{}, opts *ReadOptions) (<-chan *KVPair, error) {
...
// Watch node for changes on a "key"
// It returns a channel that will receive event data or pass
// on errors. Upon creation, the current value will first
// be sent to the channel. Providing a non-nil stopCh can
// be used to stop watching.
func (s *Zookeeper) WatchNodeEvent(key string, stopCh <-chan struct{}, runner EventRunner, opts *ReadOptions) chan error {
	// Catch zk notifications and fire changes into the channel.
	errCh := make(chan error)
	go func() {
		var fireEvt = true
		for {
			_, _, eventCh, err := s.getW(key)
			if err != nil {
				errCh <- err
			}
			select {
			case e := <-eventCh:
				...
				// Simply reset the watch if this is any other event
				// (e.g. a session event).
				fireEvt = e.Type == zk.EventNodeDeleted
				if fireEvt {
					runner.Process(&EventData{
						Key:   key,
						Event: e,
					})
				}
...
// be used to stop watching.
func (s *Zookeeper) WatchTree(directory string, stopCh <-chan struct{}, opts *ReadOptions) (<-chan []*KVPair, error) {
	// Catch zk notifications and fire changes into the channel.
	watchCh := make(chan []*KVPair)
	go func() {
		defer close(watchCh)
...
func (s *Zookeeper) List(directory string, opts *ReadOptions) ([]*KVPair, error) {
	...
	if previous != nil {
		...
		if err != nil {
			...
			if err == zk.ErrNoNode {
				...
				parts := SplitKey(strings.TrimSuffix(key, "/"))
...
func (s *Zookeeper) NewLock(key string, options *LockOptions) (lock Locker, err error) {
	value := []byte("")
...
func (s *Zookeeper) getListWithPath(path string, keys []string, opts *ReadOptions) ([]*KVPair, error) {
	kvs := []*KVPair{}
...
func (s *Zookeeper) getList(keys []string, opts *ReadOptions) ([]*KVPair, error) {
	kvs := []*KVPair{}
