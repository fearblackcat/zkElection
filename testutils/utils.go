...
func RunTestCommon(t *testing.T, kv zkLibs.Store) {
...
func RunTestListLock(t *testing.T, kv zkLibs.Store) {
	testListLockKey(t, kv)
}

// RunTestAtomic tests the Atomic operations by the K/V
// backends
func RunTestAtomic(t *testing.T, kv zkLibs.Store) {
	testAtomicPut(t, kv)
...
// by the K/V backends.
func RunTestWatch(t *testing.T, kv zkLibs.Store) {
	testWatch(t, kv)
	testWatchTree(t, kv)
}
...
// by the K/V backends.
func RunTestLock(t *testing.T, kv zkLibs.Store) {
	testLockUnlock(t, kv)
}

// RunTestLockTTL tests the KV pair Lock with TTL APIs supported
// by the K/V backends.
func RunTestLockTTL(t *testing.T, kv zkLibs.Store, backup zkLibs.Store) {
	testLockTTL(t, kv, backup)
}

// RunTestTTL tests the TTL functionality of the K/V backend.
func RunTestTTL(t *testing.T, kv zkLibs.Store, backup zkLibs.Store) {
	testPutTTL(t, kv, backup)
}

func checkPairNotNil(t *testing.T, pair *zkLibs.KVPair) {
...
func testWatch(t *testing.T, kv zkLibs.Store) {
	key := "testWatch"
	value := []byte("world")
	newValue := []byte("world!")
...
func testWatchTree(t *testing.T, kv zkLibs.Store) {
	dir := "testWatchTree"
...
func testAtomicPut(t *testing.T, kv zkLibs.Store) {
	key := "testAtomicPut"
	value := []byte("world")
	...
	assert.Equal(t, err, zkLibs.ErrKeyModified)
	assert.False(t, success)
}

func testAtomicPutCreate(t *testing.T, kv zkLibs.Store) {
	...
	success, _, err = kv.AtomicPut(key, value, nil, nil)
	assert.Equal(t, zkLibs.ErrKeyExists, err)
	assert.False(t, success)
...
func testAtomicPutWithSlashSuffixKey(t *testing.T, kv zkLibs.Store) {
...
func testAtomicDelete(t *testing.T, kv zkLibs.Store) {
...
func testLockUnlock(t *testing.T, kv zkLibs.Store) {
	key := "testLockUnlock"
	value := []byte("bar")

	// We should be able to create a new lock on key
	lock, err := kv.NewLock(key, &zkLibs.LockOptions{Value: value, TTL: 2 * time.Second})
	assert.NoError(t, err)
...
func testLockTTL(t *testing.T, kv zkLibs.Store, otherConn zkLibs.Store) {
	key := "testLockTTL"
	value := []byte("bar")
	...
	// We should be able to create a new lock on key
	lock, err := otherConn.NewLock(key, &zkLibs.LockOptions{
		Value:     value,
		TTL:       2 * time.Second,
		RenewLock: renewCh,
	})
...
func testPutTTL(t *testing.T, kv zkLibs.Store, otherConn zkLibs.Store) {
	firstKey := "testPutTTL"
	firstValue := []byte("foo")
	...
	err := otherConn.Put(firstKey, firstValue, &zkLibs.WriteOptions{TTL: 2 * time.Second})
	assert.NoError(t, err)

	// Put a second key with the Ephemeral flag
	err = otherConn.Put(secondKey, secondValue, &zkLibs.WriteOptions{TTL: 2 * time.Second})
	assert.NoError(t, err)
...
func testList(t *testing.T, kv zkLibs.Store) {
	parentKey := "testList"
	childKey := "testList/child"
	subfolderKey := "testList/subfolder"

	// Put the parent key
	err := kv.Put(parentKey, nil, &zkLibs.WriteOptions{IsDir: true})
	assert.NoError(t, err)
	...
	err = kv.Put(subfolderKey, []byte("second"), &zkLibs.WriteOptions{IsDir: true})
	assert.NoError(t, err)
...
func testListLockKey(t *testing.T, kv zkLibs.Store) {
	listKey := "testListLockSide"

	err := kv.Put(listKey, []byte("val"), &zkLibs.WriteOptions{IsDir: true})
	assert.NoError(t, err)

	err = kv.Put(listKey+"/subfolder", []byte("val"), &zkLibs.WriteOptions{IsDir: true})
	assert.NoError(t, err)
	...
	for i := 1; i <= 3; i++ {
		...
		lock, err := kv.NewLock(key, &zkLibs.LockOptions{Value: []byte("locked"), TTL: 2 * time.Second})
		assert.NoError(t, err)
		assert.NotNil(t, lock)
...
func testDeleteTree(t *testing.T, kv zkLibs.Store) {
...
func RunCleanup(t *testing.T, kv zkLibs.Store) {
	for _, key := range []string{
		...
		assert.True(t, err == nil || err == zkLibs.ErrKeyNotFound, fmt.Sprintf("failed to delete tree key %s: %v", key, err))
		err = kv.Delete(key)
		assert.True(t, err == nil || err == zkLibs.ErrKeyNotFound, fmt.Sprintf("failed to delete key %s: %v", key, err))
	}
}
